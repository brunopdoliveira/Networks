...
ip tuntap add mode tap tap_$VM_NAME
ip link set tap_$VM_NAME up

echo "Criando a vm ${VM_NAME} com a interface: "
echo "tap_${VM_NAME} : bb:00:00:11:76:${2}"

qemu-system-x86_64 -m 128 $ENABLED_KVM \
	-cpu host \
	-drive file=/vms-kvm/discos/${VM_NAME}.qcow2 \
	-netdev type=tap,ifname=tap_$VM_NAME,id=net0,script=no \
	-device virtio-net-pci,netdev=net0,mac=aa:00:00:11:76:${2} \
    -display none \
